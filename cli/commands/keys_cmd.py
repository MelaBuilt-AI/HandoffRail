...
@click.pass_context
def keys_create(ctx: click.Context, name: str, tenant_id: str | None, output_format: str) -> None:
    ...
    try:
        result = client.create_api_key(name=name, tenant_id=tenant_id)
...
def _format_keys_table(keys: list) -> None:
    ...
    click.echo(f"{'ID':<38} {'NAME':<20} {'PREFIX':<10} {'REVOKED':<8} {'CREATED'}")
    click.echo("-" * 100)
    for k in keys:
        revoked_str = "✓" if k.revoked else "─"
        click.echo(f"{k.id} {k.name:<20} {k.key_prefix:<10} {revoked_str:<8} {k.created_at}")
